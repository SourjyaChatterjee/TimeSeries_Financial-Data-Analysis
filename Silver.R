...
silver <- read_excel('silver.xlsx') # Locatoon of the DataSet
nfty50 <- silver[,c(1,2)]

nfty50$Date <- mdy(nfty50$Date)

par(mfrow = c(1,1))
plot(nfty50, ylab="Silver Prices",type = 'l')
tso <- zoo(rev(nfty50$Price/10), rev(nfty50$Date))
plot(as.Date(index(tso)),coredata(tso), ylab="Silver Prices", xlab = "Time",type = 'l',main="Silver Price in INR")

####################
#####log return#####
####################
Return=CalculateReturns(tso, method = 'log')
plot(Return,main='Return of Silver',xlab='Time')
Return
plot(Return^2,main='Sqd. Return of Silver',xlab='Time')
silver_return = Return
#####################################
...
a<- ggAcf(abs(na.omit(as.vector(Return))), col='red',main='Acf of Absolute Return ')
p<- ggPacf(abs(na.omit(as.vector(Return))),col='steelblue',main='PAcf of Absolute Return ')
...
silver_result <- data.frame(mean_first,mean_second,var_first,var_second,aic,bic,aicc)
silver_result
colnames(silver_result) <- c("1st mean Parameter","2nd mean Parameter","1st variance Parameter","2nd variance Parameter","AIC Scores","BIC Scores","AICc Scores")
write.csv(silver_result,'Silver_model.csv')

which(silver_result$`AIC Scores` == min(silver_result$`AIC Scores`))
which(silver_result$`BIC Scores` == min(silver_result$`BIC Scores`))
which(silver_result$`AICc Scores` == min(silver_result$`AICc Scores`))

#####################################################################################
#####################################################################################

#so, our final model is arma(0,0)-garch(0,1) according to aic score
n50_garch_1 <- ugarchspec(mean.model = list(armaOrder=c(0,0)),variance.model = list(model = 'sGARCH', 
                                                                                    garchOrder = c(1,1)),distribution = 'std')
fit_garch_1 <- ugarchfit(spec = n50_garch_1, data= na.omit(silver_return))

coef(fit_garch_1)

#so, our final model is arma(3,2)-garch(3,0) according to aic & bic score

n50_garch_2 <- ugarchspec(mean.model = list(armaOrder=c(3,2)),variance.model = list(model = 'sGARCH', 
                                                                                    garchOrder = c(3,0)), distribution = 'std')
fit_garch_2 <- ugarchfit(spec = n50_garch_2, data= na.omit(silver_return))
...
fit_roll <- ugarchfit(n50_garch_2, data= na.omit(Return),out.sample =50)
